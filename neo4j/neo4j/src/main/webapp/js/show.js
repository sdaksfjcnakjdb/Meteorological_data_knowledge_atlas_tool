 ...
 * 查询功能调用方法
 * */
function findbyscene(){
    //没有选择标签
    if(document.getElementsByClassName("ant-select-selection-item").length ==0){
        alert("请选择标签！")
    }else {
        document.getElementById("loading").style.setProperty("display","block")
        var scene = $("input[name='scene']")[0].value;
        var elements = $("input[name='elements']")[0].value;
        var label = document.getElementsByClassName("ant-select-selection-item")[0].innerHTML;
        console.log(label);
        $.ajax({
            type: "Post",//请求方式
            url: "action",//地址，就是json文件的请求路径
            data: {"action": "queryByScene", "scene": scene, "elements": elements, "label": label},
            dataType: "json",//数据类型可以为 text xml json  script  jsonp
            async: false,
            success: function (result) {//返回的参数就是 action里面所有的有get和set方法的参数
                success(result,2);//默认为2
            }
        });
    }
}
...
function change01(){
    find01 = document.getElementById("find01");
    find02 = document.getElementById("find02");

    find01.style.display = "none"
    find02.style.display = "block"
}
...
function change02(){
    ...
    find01.style.display = "block"
    find02.style.display = "none"
}

/**
 * 根据传入的数据构建图谱
 * */
function success(result,depth){
    node = [];
    edge = [];
    nodeList= [];//全id集合
    colorGroup = [];
